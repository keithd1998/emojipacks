 ...
 * Module dependencies.
 */
var isChatsystem = require('./valid').chatsystem;
...
exports.start = function* (chatsystem, subdomain, email, password, pack) {
  var load, valid;

  if (!chatsystem) {
    chatsystem = yield ask('Chatsystem: ', isSupported, 'Uh oh! Looks like you have an unsupported chat system!');
  }
  if (!subdomain) {
 ...
 *  is supported
 */
function isSupported(chatsystem) {
  return chatsystem === 'slack' || chatsystem === 'mattermost';
}
